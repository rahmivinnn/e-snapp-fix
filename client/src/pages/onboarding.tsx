    ...
    <div className="h-screen bg-white w-full max-w-sm mx-auto flex flex-col overflow-hidden">
      ...
      <div className="bg-white px-6 pb-6 pt-2 flex flex-col justify-end">
        ...
        <div className="space-y-3 mb-4">
          <Button 
            onClick={handleNext}
            className="w-full bg-primary hover:bg-primary/90 text-white py-3 text-base font-medium transition-all duration-200 hover:scale-[1.02] rounded-xl shadow-lg"
          >
            Next
          </Button>
          ...
          <button
            onClick={handleSkip}
            className="w-full text-primary py-2 text-base font-medium hover:text-primary/80 transition-colors"
          >
            Skip
          </button>
        </div>
          ...
          {onboardingSteps.map((_, index) => (
            <div
              ...
              className={`w-2 h-2 rounded-full transition-all duration-300 ${
                index === currentStep ? 'bg-primary w-6' : 'bg-gray-300'
              }`}
            />
          ))}
        </div>
